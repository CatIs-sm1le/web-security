...
<body>
   <script type='text/javascript'> $(function(){ $('.level-form').submit(function(e){ if(document.getElementById('login').value == 'frontend' && document.getElementById('pass').value == 'backend') { } else { e.preventDefault(); alert('Incorrect login') } })})</script>
    ...
    <div class="row">
      ...
      <div class="col-md-8 spoiler-body" id="text">
        <p class="spoiler-body">
          Использование JavaScript в качестве единственного метода для защиты вашего сайта - плохая идея, но это, очевидно, было упущено при написание этой страницы.
        </p>
      </div>
    </div>
  </main>
